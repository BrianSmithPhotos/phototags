...
| `test_exif_service.py` | `exif_service.py`: `map_for_ui` field mapping, GPS DMS/decimal coordinate parsing and range validation, altitude formatting, `ArtFilterEffect`/`PictureMode`/`StackedImage`/`MultipleExposureMode` fallback chain, aperture formatting, `read_full_metadata` against a mocked `subprocess.run` (success, nonzero exit, invalid JSON, empty result), `read_full_metadata_for_paths`/`_read_chunk` (single batched call, per-file fallback on a path missing from the batch result, fallback on batch timeout, individual fallback failure surfacing as a value not an exception, chunk-boundary splitting) | These mapping/parsing rules feed the metadata panel and the rename/AI flows downstream; a wrong fallback or a coordinate-parsing bug shows the wrong (or no) value in the UI without erroring. The batched-read fallback logic is what keeps one slow/unreadable file in an import batch from silently losing metadata for every other file batched alongside it. |
...
| `test_metadata_write_service.py` | `metadata_write_service.py`: `write_description_keywords` against a mocked `subprocess.run` (success, exiftool-command GPS argument construction, write failure restoring the `_original` backup file), GPS validation error paths (missing longitude, out-of-range latitude), `write_description_keywords_for_paths` (single batched call across files, fallback to per-file writes on batch failure, individual fallback failure surfacing as a value not an exception, backup restoration for the whole group before falling back) | Metadata writes are the one operation that mutates files on disk; a wrong command argument or a skipped backup-restore on failure means silent data loss or corruption. The batched-write fallback logic is what keeps a failed group write from leaving any file's backup unrestored. |
...
246 tests, all currently passing, ~0.5s total.
